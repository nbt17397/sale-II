...
class ObjecyiveDetailViewOnly extends Component {
    constructor(props) {
        super(props);
        this.state = {
            field_of_view: {},
    ...
    //     const { companyObjective, name, starting_date, ending_date, business_unit_id, branch_id, type } = this.state
    ...
    async init() {
        const { loadViews, params, setLoading, user, searchRead, showDialog, id } = this.props
        setLoading(true)
    ...
    render() {
        const { name, type, listType, branch_id, listBranch, business_unit_id, listBusiness, starting_date,
            ending_date, line_ids, companyObjective, logActive, error, isEditing, detailInfo, open, is_verified,
        } = this.state
...
export default function (props) {
    ...
    const { showDialog, setLoading, read, create, unlink, write, loadViews, nameSearch, searchRead,
        showNotification, getControlIndexFilter, getRevenuesFilter, getSellingExpensessFilter, getManagementExpensesFilter } = useContext(UserContext);
    ...
    const [breadcrumb] = useState([{ name: 'Mục tiêu công ty', path: routes.objectiveView.path }])
    ...
    return <ObjecyiveDetailViewOnly {...props} {...{
        ...
        getSellingExpensessFilter, getManagementExpensesFilter, columns, setData, dataMemo
    }} />;
}